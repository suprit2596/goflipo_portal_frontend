...
const CreateBusiness = ({ business, isEditMode = false, onSuccess }) => {
  ...
  const formik = useFormik({
    initialValues: {
      ...
      websiteUrl: business?.websiteUrl || '',
      accountStatus: business?.accountStatus || BUSINESS_STATUS.ACTIVE,
      authType: business?.authType || 'IN_APP',   // ✅ default
    },
    enableReinitialize: true,
    validationSchema,
    onSubmit: async (values) => {
          ...
          <div style={{ padding: '16px 24px' }}>
            <form onSubmit={formik.handleSubmit}>
              ...
              >
                ...
                {/* 🔐 AUTH TYPE (Catchy Toggle Cards) */}
                <div style={{ gridColumn: '1 / 3' }}>
                  <label style={{ fontSize: 13, fontWeight: 600 }}>Authentication Type</label>

                  <div style={{ display: 'flex', gap: 18, marginTop: 10 }}>

                    {/* IN APP */}
                    <div
                      onClick={() => formik.setFieldValue('authType', 'IN_APP')}
                      style={{
                        flex: 1,
                        padding: '14px 18px',
                        borderRadius: 12,
                        border: formik.values.authType === 'IN_APP'
                          ? '2px solid #3182ce'
                          : '1px solid #e2e8f0',
                        background: formik.values.authType === 'IN_APP'
                          ? 'linear-gradient(135deg, #ebf4ff, #ffffff)'
                          : '#f8fafc',
                        cursor: 'pointer',
                        transition: 'all 0.2s ease',
                        boxShadow: formik.values.authType === 'IN_APP'
                          ? '0 8px 18px rgba(49,130,206,0.18)'
                          : 'none',
                      }}
                    >
                    ...
                    {/* TOTP */}
                    <div
                      onClick={() => formik.setFieldValue('authType', 'TOTP')}
                      style={{
                        flex: 1,
                        padding: '14px 18px',
                        borderRadius: 12,
                        border: formik.values.authType === 'TOTP'
                          ? '2px solid #3182ce'
                          : '1px solid #e2e8f0',
                        background: formik.values.authType === 'TOTP'
                          ? 'linear-gradient(135deg, #ebf4ff, #ffffff)'
                          : '#f8fafc',
                        cursor: 'pointer',
                        transition: 'all 0.2s ease',
                        boxShadow: formik.values.authType === 'TOTP'
                          ? '0 8px 18px rgba(49,130,206,0.18)'
                          : 'none',
                      }}
                    >
